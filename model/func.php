...
{
	extract($data);
	$sql = "INSERT INTO tender (prod,qty,unit,min_price,max_price,date_due,approval_date,bid_date,priority) VALUES (\"$prod\",\"$qty\",\"$unit\",\"$min_price\",\"$max_price\",\"$date_due\",\"$appr\",\"$bid_date\",\"$priority\")";
	return addData($conn,$sql);
}
...
{
	$sql = "SELECT p.pic, p.category, t.id, t.prod, t.qty, t.unit, t.min_price, t.max_price, t.date_due, t.bid_date, t.priority, t.state FROM tender t, products p WHERE p.name = t.prod AND t.state = $state AND P.category=\"$cat\"";
	return fetchData($conn,$sql);
}
...
{
	$id = $_SESSION['id'];
	$sql = "SELECT * FROM staffdetailstble WHERE email <> \"sudo@sys\.com\" AND idno <> \"$id\"";
